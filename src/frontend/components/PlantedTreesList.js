...
const PlantedTrees = ({ marketplace, userAddress }) => {
  ...
  return (
    <Container>
      <h2 className="mb-4">Planted Trees</h2>
      <Row>
        {trees.map((tree) => (
          <Col md={4} key={tree.tokenId.toNumber()}>
            <Card className="mb-4">
              <Card.Body>
                <Card.Title className="mb-3">Owner: {tree.donor}</Card.Title>
                <Card.Text>
                  <strong>Token ID:</strong> {tree.tokenId.toNumber()}
                </Card.Text>
                <Card.Text>
                ...
                <Card.Text>
                  ...
                  {ethers.utils.formatEther(tree.amount)}
                </Card.Text>
